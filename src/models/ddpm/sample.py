#!/usr/bin/env python3
"""
Sampling script for DDPM - Modular version.
...
Usage:
    python sample.py --checkpoint outputs/.../checkpoint_100.pt --config configs/ddpm_default.yaml --mode unconditional
"""
...
def load_model_from_checkpoint(checkpoint_path, config_path=None, device='cuda'):
    ...
    Args:
        checkpoint_path: Path to checkpoint file (e.g., checkpoint_100.pt)
            ...
            if args_path.exists():
                ...
                config = {
                    'model': {
                        ...
                        'objective': getattr(args, 'objective', 'pred_v'),
                        'beta_schedule': getattr(args, 'beta_schedule', 'sigmoid'),
...
def main():
    parser = argparse.ArgumentParser(description='Sample from DDPM model - Modular version')
    ...
    if args.output_dir:
        ...
        save_root = resolve_path(config['paths'].get('samples_dir', 'samples/ddpm'))
