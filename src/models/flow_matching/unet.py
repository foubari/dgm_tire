"""
UNet architecture for Flow Matching with Classifier-Free Guidance support
"""
...
class Unet(Module):
    """
    UNet architecture with Classifier-Free Guidance support.
    
    Args:
        dim: Base dimension
        init_dim: Initial dimension (default: dim)
        out_dim: Output dimension (default: channels)
        dim_mults: Channel multipliers (default: (1, 2, 4, 8))
        channels: Input channels (default: 3)
        self_condition: Use self-conditioning (default: False)
        learned_variance: Learn variance (default: False)
        ...
        sinusoidal_pos_emb_theta: Theta for sinusoidal pos emb (default: 10000)
        dropout: Dropout rate (default: 0.)
        attn_dim_head: Attention dimension per head (default: 32)
        attn_heads: Number of attention heads (default: 4)
        full_attn: Full attention pattern (default: None, auto)
        flash_attn: Use flash attention (default: False)
        cond_dim: Conditioning dimension (default: 2 for width_px, height_px)
    """
    ...
    def __init__(
        ...
        out_dim=None,
        dim_mults=(1, 2, 4, 8),
        channels=5,  # Default: 5 components stacked
        ...
        full_attn=None,
        flash_attn=False,
        cond_dim=2,  # Default: width_px, height_px
    ):
        super().__init__()
    ...
    def forward(self, x, time, *, cond=None, x_self_cond=None):
        ...
        if exists(self.cond_mlp) and cond is not None:
